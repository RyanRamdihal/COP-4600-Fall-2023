...
# 1. Scheduler functions (AI-written unless otherwise specified) were moved from their own conversations/files to this
#    one.
# 2. Lines explicitly marked "human-fixed" were fixed for the reasons stated in each line.
...
def sjf(processes, runFor, output_file):
    # Human-fixed removed unnecessary sort that was preventing arrivals from being shown in the order they were input.
    current_time = 0
    ...
    with open(output_file, 'w') as file:
        ...
        while current_time < runFor:
            ...
            # Human-fixed Move "finished" events to match output test files
...
def fifo(processes, runFor, output_file):
    ...
    with open(output_file, "w") as f:
        ...
        f.write("Using First-Come First-Served\n") # Human-fixed fixed name

        while current_time < runFor: # Human-fixed Remove "or queue" that was required all processes to finish before ending
            # Check for arriving processes and add them to the queue
            for process in processes:
                if process.arrival == current_time:
                    queue.append(process)
        ...
        printMetrics(processes, f) # Human-fixed use existing printMetrics function instead of recalculating
...
def rr(processes, run_for, quantum, output_file):
    ...
    with open(output_file, 'w') as output:
        ...
        while current_time < run_for: # Human-fixed removed "or queue" , because it would continue iterating after runfor
            #ARRIVED
            for process in processes:
                if process.arrival == current_time:
                    # Human-fixed modify this section to work with re-ordered code
                    queue.append(process)
            ...
            # Human-fixed moved process preempting code after arrivals
            ...
            if queue and quantum_counter == quantum:
                ...
                # Human-fixed deselect the process so the selection gets printed again
            ...
            if queue:
                ...
                # Human-fixed modified condition to only print when the quantum is zero, so we print only when the 
...
def main():
    ...
    processes = []
    run_for = None # Human-fixed removed default value for run_for so we can check if it was provided
    scheduler = None
    quantum = None # Human-fixed removed defaut value for quantum
    num_processes = None # Human-fixed initialize num_processes so we can check if it was provided
    scheduler_provided = False  # Variable to track if the scheduler parameter was provided
    
    # Human-fixed moved lambda to different scope so we can check the selected scheduler later
    ...
    try:
        with open(input_file, 'r') as file:
            for line in file:
                ...
                # Human-fixed prevent crashes on empty lines in input file
                if len(parts) == 0:
                    directive = None
                else:
                    directive = parts[0]
                ...
                if directive == "processcount":
                    ...
                    algorithm = parts[1]
                    scheduler_provided = True # Human-fixed moved line to avoid repeated code
                    if algorithm == "sjf":
                        scheduler = sjf
                    elif algorithm == "fcfs":
                        scheduler = fifo
                    elif algorithm == "rr":
                        # Human-fixed fixed argument order and count
    ...
    if not scheduler_provided:
        print("Error: Missing parameter - use") # Human-fixed fixed error message
        sys.exit(1)

    if scheduler == rr_scheduler and not quantum: # Human-fixed check scheduler against the lambda, not the original function
        print("Error: Missing parameter - quantum") # Human-fixed fixed error message
        sys.exit(1)
