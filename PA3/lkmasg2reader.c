...
#define DEVICE_NAME "lkmasg2reader" // Device name.
#define CLASS_NAME "char_read"	  ///< The device class -- this is a character device driver

#define BUFFER_SIZE 1024

MODULE_LICENSE("GPL");						 ///< The license type -- this affects available functionality
MODULE_AUTHOR("John Aedo");					 ///< The author -- visible when you use modinfo
MODULE_DESCRIPTION("lkmasg2 Reader Module"); ///< The description -- see modinfo
...
{
	printk(KERN_INFO "lkmasg2 Reader: installing module.\n");
	...
	printk(KERN_INFO "lkmasg2 Reader: registered correctly with major number %d\n", major_number);
	...
	printk(KERN_INFO "lkmasg2 Reader: device class registered correctly\n");
...
void cleanup_module(void)
{
	...
	printk(KERN_INFO "lkmasg2 Reader: Goodbye from the LKM!\n");
	unregister_chrdev(major_number, DEVICE_NAME);
	return;
}
...
static ssize_t read(struct file *filep, char *buffer, size_t len, loff_t *offset)
{
	...
	mutex_lock(&lkmasg2_mutex); // Lock the mutex before entering the critical section
	...
	if (readBytes > BUFFER_SIZE) {
		...
		printk(KERN_INFO "lkmasg2 Reader - Buffer has %d bytes of content, requested %d.", availableBuffer, len);
	}
	...
	if (readBytes > 0)
    	printk(KERN_INFO "lkmasg2 Reader - Read %d bytes from the buffer.", readBytes);

	mutex_unlock(&lkmasg2_mutex); // Unlock the mutex after exiting the critical section
